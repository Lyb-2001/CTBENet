...
def standard_batch_cka(feat_s, feat_t):
    """
    计算标准的、基于样本的CKA。
    输入为整个batch的特征，shape [B, C, H, W] 或 [B, D]。
    返回一个标量，代表整个batch的表征相似度。
    """
    # 1. 将特征展平为 [B, D]
    # B是样本数，D是每个样本的特征维度
    x = feat_s.flatten(1)  # shape: [B, C*H*W]
    y = feat_t.flatten(1)  # shape: [B, C*H*W]

    # 2. 计算Gram矩阵，此时Gram矩阵是 [B, B] 维的
    # 这衡量的是样本之间的相似性
    ...
    # 4. 计算CKA
    # (gx * gy).sum() 是 HSIC 的一个计算方式，等价于 trace(gx.T @ gy)
    cka_val = (gram_x_centered * gram_y_centered).sum() / (
        torch.norm(gram_x_centered, p='fro') * torch.norm(gram_y_centered, p='fro') + 1e-8
    )
...
#         """
#         初始化 Spectral Gating Network 层。
#
#         Args:
#             dim (int): 输入特征的通道数。
#         """
#         super().__init__()
#         self.dim = dim  # 保存维度信息，虽然在当前逻辑下可能不是必须的，但良好实践
#         # self.sigmoid = nn.Sigmoid()
#         # self.conv = conv1x1_bn_relu(dim, dim) # 如果这个卷积层确实不用，可以注释掉或删除
#
#         # 将 complex_weight 初始化为 None，表示尚未创建
#         # 它将在第一次前向传播时根据输入尺寸被创建
#         self.complex_weight = None
#         self.conv = conv1x1_bn_relu(dim,dim)
#
#     # 不再需要 _init_weight 方法
#
#     def forward(self, x):
#         """
#         前向传播函数。
...
#
#         # --- 懒惰初始化 ---
#         # 检查 complex_weight 是否已经被初始化
#         if self.complex_weight is None:
#             # 如果未初始化 (第一次调用 forward)
#             # 创建一个形状为 (C, H, W, 2) 的随机张量，最后一个维度代表实部和虚部
#             # 注意：权重通常不依赖于批次大小 B，所以形状是 (C, H, W, 2)
#             # 设备应与输入 x 相同，这样模块才能在不同设备（CPU/GPU）上工作
#             real_imag_weight = torch.randn(C, H, W, 2, dtype=torch.float32, device=x.device)
#             # 将其注册为模型参数
#             self.complex_weight = nn.Parameter(real_imag_weight)
#         elif self.complex_weight.shape[:3] != (C, H, W):
#              # 可选：如果后续输入的 C, H, W 发生变化，这里可以报错或采取其他策略
#              # 对于大多数固定结构的模型，这里的 C, H, W 在第一次之后应该是固定的
#              raise ValueError(f"Input shape changed unexpectedly! "
#                               f"Expected (C, H, W) = {self.complex_weight.shape[:3]}, "
#                               f"Got {(C, H, W)}")
#         # --- 初始化结束 ---
#
#         # 执行 2D FFT
...
#
#         # 从存储的实数张量 (C, H, W, 2) 创建复数权重视图 (C, H, W)
#         # 注意：complex_weight 现在是 nn.Parameter，可以直接使用
#         weight = torch.view_as_complex(self.complex_weight)
#
#
#
#
#         x_gated = x_fft * weight + x_fft # 保持原始逻辑，但需注意 sigmoid 部分
#
#         # 执行逆 2D FFT，并取实部（假设原始输入 x 是实数）
#         x_out = torch.fft.ifft2(x_gated, dim=(-2, -1), norm='ortho').real
#
...
class FASML1(nn.Module):
    ...
    def calculate_feature_difficulty2(self, f1, f2):
        ...
        similarity_score = standard_batch_cka(f1,f2).cuda()
