...
class WarmupPolyLR(_LRScheduler):
    def __init__(self, optimizer, T_max, warmup_factor=1.0/3, warmup_iters=500,  # 移除 cur_iter 参数
                 eta_min=0, power=0.9):
        self.warmup_factor = warmup_factor
        self.warmup_iters = warmup_iters
        self.power = power
        ...
        self.cur_iter = 0  # 内部维护迭代次数
        super().__init__(optimizer)

    def step(self):  # 重写 step 方法
        self.cur_iter += 1  # 每次调用 step() 时自动递增
        super().step()  # 调用父类方法触发学习率计算
